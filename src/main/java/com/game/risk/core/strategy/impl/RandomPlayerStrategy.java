...
public class RandomPlayerStrategy implements PlayerStrategy {
	...
	public void reinforce() {
		while (player.getNumberOfArmies() > 0) {
			Random random = new Random(player.getNumberOfArmies() + 1);
			int randomArmies = random.nextInt();
			Random random1 = new Random(player.getCountriesOwned().size());
			int randomCountryIndex = random1.nextInt();
	...
	@Override
	public void attack() {
		Random rounds = new Random(5);
		int noOfRounds = 1 + rounds.nextInt();
		while (noOfRounds > 0) {
			Random random = new Random(player.getCountriesOwned().size());
			Country attacker = player.getCountriesOwned().get(random.nextInt());
			...
			if(attacker.getCurrentNumberOfArmies() >= 2) {
				...
				if (defenderList.isEmpty()) {
					...
					Random random1 = new Random(defenderList.size());
					Country defender = defenderList.get(random1.nextInt());
	...
	public void fortify() {
		...
		Random random = new Random(player.getCountriesOwned().size());
		int randomCountry1Index = random.nextInt();
		int randomCountry2Index = random.nextInt();
		...
		Random random1 = new Random(country1.getCurrentNumberOfArmies());
		int randomArmies = random1.nextInt();

		while (!countriesGraph.getAdjListHashMap().get(country1).contains(country2)) {
			randomCountry2Index = random.nextInt();
	...
	private int getRandomDice(Country attacker, int maxDice) {
		...
		if (attackerArmies <= maxDice) {
			random = new Random(attackerArmies-1);
		} else {
			random = new Random(maxDice);
		}
		int diceCount = 1 + random.nextInt();
		return diceCount;
	}
